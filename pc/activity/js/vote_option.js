...
$(function(){
    ...
    $("input[name='voteId']").val(GetQueryString("voteId"));
    var id = GetQueryString("id") ;//alert(activityid);
    if(id){ 
        // 请求文章内容
        $.ajax({
    ...
    $("#submit").bind("click",function(){ 
        ...
        if($("input[name='name']").val() == '' || $("input[name='content']").val() == ''){
        	alert("请检查数据不能为空");
        }else{
		
                $.post("../php/vote.php?handle=newItem",
					 $("form").serialize(),
					 function(data) {  
                        console.log(data);
                        if(data == 1){
